...
| OCI chart repo on GHCR | **Real, and current at `0.2.0` (§5.5).** `oci://ghcr.io/ananttyagi07/charts/{kharcha,chidrixx-controlplane}` — packaged, pushed, pulled back down anonymously, and `helm template`d from the round-tripped artifact to confirm it's not corrupted. Charts and images are released together by `.github/workflows/release.yml`, and `image.tag` derives from the chart's own `appVersion`, so a published chart can no longer deploy an image it wasn't released with. |
